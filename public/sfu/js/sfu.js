...
(function ($) {
  ...
  $('footer')
    .html(
      '<a href="http://www.sfu.ca/canvas"><img alt="SFU Canvas" src="/sfu/images/sfu-logo.png" width="250" height="38"></a>'
    )
    .show();
  ...
  utils.onPage(/^\/profile\/settings\/?$/, () => {
    $(document).ready(() => {
      const $fieldsToLock = $(
        '.full_name.display_data, .sortable_name.display_data'
      );
      const $helpText = $('.short_name').siblings('span.edit_or_show_data');
      ...
      $helpText.append(
        '<br />Changing this will only affect your display name within Canvas, ' +
      ...
      $('#update_profile_form')
        .find('label[for="user_subscribe_to_emails"]')
        .parents('tr')
        .hide();
    });
  });
  ...
  utils.onPage(/^\/register\/\w+/, () => {
    $(document).ready(() => {
      $('#registration_confirmation_form')
        .find('label[for="user_subscribe_to_emails"]')
